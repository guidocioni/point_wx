...
def make_temp_prec_climate_figure(df, title=None):
    ...
    fig.update_yaxes(
        row=1, col=1, secondary_y=True, title="mm", showgrid=False, fixedrange=True
    )
    fig.update_yaxes(
        row=1, col=1, secondary_y=False, title="°C", showgrid=False, fixedrange=True
    )
...
def make_wind_rose_figure(df, title=None):
    def degree_to_direction(degree):
        ...
        index = round(degree / (360.0 / len(directions))) % len(directions)
    ...
    for i, group in df.groupby(df.time.dt.month):
        frequencies = (
            pd.cut(group["wind_direction_10m_dominant"], bins=np.linspace(0, 360, 17))
            .value_counts(sort=False)
            .to_frame()
            .reset_index()
        )
        frequencies["wind_direction_10m_dominant"] = frequencies[
    ...
    fig.update_layout(
        margin={"r": 2, "t": 1, "l": 2, "b": 0.1}, dragmode=False, height=150
    )
...
fig_temp_prec_climate = html.Div(
    [
        ...
        dcc.Graph(id=dict(type="figure", id="temp-prec-climate"), config=images_config),
    ],
    className="mb-2",
)
